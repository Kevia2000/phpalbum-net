...
function check_pic_order($list){

	$list=str_replace("l[]=","",$list);
	$order=split("&",$list);
...
if($var1=="params"){
	...
	ob_start();require("setup/setup_params.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
}
if($var1=="users"){
	if($var2=="delete"){
	...
	ob_start();require("setup/setup_users.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
}
if($var1=="groups"){
	if($var2=="delete"){
	...
	ob_start();require("setup/setup_groups.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
}
if($var1=="user"){
	$superuser=false;
	...
	ob_start();require("setup/setup_user.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
}
if($var1=="themes"){
	$cfg=theme_get_theme_cfg_xml($pa_theme["theme_path"]."theme.xml");
	if($var2=="add_color"){
		$seq=db_get_seq_nextval("color_map_id");
		$colors=Array();
		foreach($cfg["parameter"] as $name=>$value){
			if($value["type"]=="color"){
				$colors[$name]=$value["default"];
			}
		}
		if(!$demo_mode)db_insert("color_map",Array("theme"=>$pa_theme["name"],"id"=>$seq,"colors"=>$colors));
	...
	if($var2=="save"){
			if($_POST["p_theme"]!=$pa_setup["site_theme"]){
				...
				/*adding custom theme parameters*/
				$col_map=Array();
				foreach($cfg["parameter"] as $pos => $val){
					/*checkboxes*/
					if($val["type"]=="checkbox"){
						if(isset($_POST["p_cs_".$pos])){
							$set.="custom_parameters['$pos']='".$val["checked"]."';";
						}else{
							$set.="custom_parameters['$pos']='".$val["unchecked"]."';";
						}
					}else if($val["type"]=="color"){
						$col_map[$pos]=$_POST["p_cs_".$pos];
					}else{
						$set.="custom_parameters['$pos']='".$_POST["p_cs_".$pos]."';";
					}
				}
				
				if(!$demo_mode)db_update("theme",$set,"name=='".$pa_setup["site_theme"]."'");
				if(!$demo_mode)db_update("color_map","name='".$_POST["p_color_name"]."';colors=".var_export($col_map,true),"id==".$pa_color_map["id"]);
    ...
    pa_read_settings();
  	ob_start();require("setup/setup_themes.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
...
if($var1=="search"){
	...
	if($var2=="save"){
		 if(isset($_POST["p_index_keywords"])){
		 	$set="index_keywords='true';";
		 }else{
		 	$set="index_keywords='false';";
		 }
		 if(isset($_POST["p_index_short_desc"])){
		 	$set.="index_short_desc='true';";
		 }else{
		 	$set.="index_short_desc='false';";
		 }
			 if(isset($_POST["p_index_long_desc"])){
		 	$set.="index_long_desc='true';";
		 }else{
		 	$set.="index_long_desc='false';";
		 }
			 if(isset($_POST["p_index_filenames"])){
		 	$set.="index_filenames='true';";
		 }else{
		 	$set.="index_filenames='false';";
		 }
	...
	ob_start();require("setup/setup_search.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
...
if($var1=="thumbnails" ){
  	...
  	ob_start();require("setup/setup_thumbnails.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	return;
}
...
if($var1=="main"){
	  ...
	  ob_start();require("setup/setup_main.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
}
	...
	if($var1=="logs"){
	  if($var2=="save"){
		...
		pa_read_settings();
   	  ...
   	  ob_start();require("setup/setup_logs.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	if($var1=="pics"){
		if(strlen($var3)==0){
		...
		if($var2=="save"){
			$cnt=1;
			$order=check_pic_order($var4);// returns array of order of pictures
			...
			if(is_array($order)){
				if(!$demo_mode)db_update("directory","sorting='manual';","seq_files=='".$var3."'");
			}
		}
	  ob_start();require("setup/setup_photos.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	if($var1=="dirs"){
		if(strlen($var3)==0){
		...
		if($var2=="save"){
			pa_invalidate_object_cache("GNP"); //invalidate cache (what if groups has been changed)
			/*save settings*/
   	  ...
   	  ob_start();require("setup/setup_dirs.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	...
	if($var1=="comments"){
	  ...
	  if($var2=="save"){
  	  ...
  	  ob_start();require("setup/setup_comments.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	if($var1=="ecard"){
	  if($var2=="save"){
  	  ...
  	  ob_start();require("setup/setup_ecard.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	if($var1=="cache"){
	  if($var2=="delete"){
	  ...
	  if($var2=="save"){
  	  ...
  	  ob_start();require("setup/setup_cache.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	if($var1=="syscheck"){
		ob_start();require("setup/setup_syscheck.inc");
	...
	if($var1=="errorlog"){
		...
		ob_start();require("setup/setup_errorlog.inc");
		$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
	}
	if($var1=="texts"){
		...
		ob_start();require("setup/setup_texts.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
	  return;
  }
	if($var1=="admin" && !$demo_mode ){
		...
		if(!$var2){
				ob_start();require("setup/setup_admin.inc");$contents = ob_get_contents();ob_end_clean();theme_generate_setup_page($menu,$contents);
				return;
		}
		if($var2=="login"){
...
/*reading install script and doing what to do is*/
	...
	$test=md5(time());
	$f=fopen($pa_setup["cache_dir"]."pj_test","w"); fwrite($f,$test); fclose($f);
	
   	if(!ftp_put($conn,"pj_test",$pa_setup["cache_dir"]."pj_test",FTP_BINARY)){ftp_close($conn); return t("ID_MSG_FTP_UNABLE_UPLOAD","testfile");}
	
	$ff=file($pa_setup["album_dir"]."pj_test");
	if($ff[0]!=$test){
		ftp_close($conn);
		return t("ID_MSG_FTP_PHOTO_DIR");
	}
	$result=ftp_delete($conn,"pj_test");	
